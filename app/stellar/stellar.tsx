...
export async function createTransactionXDR(
    ...
    const transaction = new StellarSdk.TransactionBuilder(account, {
        ...
        .addOperation(
            StellarSdk.Operation.payment({
                destination:
                    "GDT36CKLBCIQUHJPAHSAYCGQW5CHLIGIRRCQ2R7B5ERRCR2DGV7ADO7Z", //Address of Fee receiver
                asset: usdcAsset,
                amount: (parseFloat(amountLumens) * 0.005).toString(),
            })
        )
...
export async function swapXLMtoUSDC(publicKey, amountToSwap, secretKey) {
    ...
    try {
        ...
        try {
            ...
            xlmBalance =
                sourceAccount.balances[sourceAccount.balances.length - 1]
                    .balance;
        } catch (error) {
            console.log("=> ERROR:", error);
            throw new Error("Unable to fetch account details");
        ...
        console.log(
            "CIAO",
            (xlmBalance > 0 ? xlmBalance * 0.9 : 100).toString()
        );
        let transaction;
        try {
...
async function authenticate(account: string, secretKey: string) {
    try {
        ...
        const challengeTransactionXDR = await fetchChallengeTransaction(
            account
        );
